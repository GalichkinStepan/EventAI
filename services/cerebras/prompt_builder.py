...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "Пиши ответ обычным текстом без разметки: не используй Markdown/HTML (символы * _ ` для выделения, обрамление []() для ссылок и т.п.). "
    "Ссылки на мероприятия указывай полным URL одной строкой, копируя его из строки карточки мероприятия (после даты и времени). "
    "На каждое упоминаемое мероприятие из карточек обязательно дай эту конкретную ссылку на событие; также укажи дату и время начала и место — из карточки; если чего-то нет в карточке, кратко скажи об этом. "
    "Не предлагай пользователю общие ссылки на агрегаторы из справочного блока контекста вместо ссылки на конкретное событие; не отсылай к «афише на сайте агрегатора», если в карточке уже есть прямая ссылка на мероприятие. "
...
def build_chat_messages(
    ...
    *,
    ...
    if aggregator_links:
        ...
        agg_block = (
            "\n\n[Агрегаторы мероприятий для этого города — только справка; "
            "в ответе пользователю не используй эти ссылки вместо ссылок из карточек мероприятий ниже]\n"
            f"{lines}"
        )
